    ...
    public function getPageNumber(Request $request){
        ...
        if(!empty($data['token']) && !empty($data['time']) && $data['secret']){
            ...
            $baseQuery = DB::table('devices')
                    ...
                    ->join('projects', 'projects.play_list', '=', 'play_lists.id')
                    ...
                    'play_lists.id as PID','play_lists.duration as duration','projects.id as project_id')
                    ->get();        
            ...
            if($baseQuery){
                $pageQuery = DB::table('devices')
                        ...
                        ->where('devices.secret',$data['secret'])
                        ->select('devices.*','channels.name as channel_name','play_lists.name as list_name','play_lists.name as list_name',
                            'play_lists.duration as duration','projects.id as project_id','schdules.start_time','schdules.end_time','schdules.duration as SC_duration',
                ...
                foreach($pageQuery as $item){
                    $playListArr[$item->PID][] = $item->project_id;

                    // calculate time range
                    $rowTimeArr = explode(':', $item->start_time);
                    $rowStartTime = $rowTimeArr[0] * 60 + $rowTimeArr['1'];
                    $rowEndTime = $rowStartTime + (int) $item->SC_duration;
    ...
    public function getPage(Request $request){
        ...
        $deviceID = $data['token'];
            $baseQuery = DB::table('devices')
                    ...
                    ->where('projects.published',1)
                    ->where('devices.secret',$data['secret'])
                    ->select('devices.*','channels.name as channel_name','play_lists.name as list_name','play_lists.name as list_name','play_lists.id as PID',
                    'play_lists.duration as duration','projects.id as project_id')
                    ->get();        
            ...
            if($baseQuery){
                ...
                $pageQuery = DB::table('devices')
                        ...
                        ->where('devices.secret',$data['secret'])
                        ->select('devices.*','channels.name as channel_name','play_lists.name as list_name','play_lists.name as list_name',
                        'play_lists.duration as duration','projects.id as project_id','projects.design as page_html','schdules.start_time','schdules.end_time','schdules.duration as SC_duration',
                        'play_lists.duration as l_duration','play_lists.id as PID')
                        ->get();
                ...
                /// Makeing Play list to projects array. also checking what play list we need to play;
                foreach($pageQuery as $item){
                    $playListArr[$item->PID][] = $item->page_html;//$item->project_id;
                ...
                foreach($playListArr[$PlayListID] as $project){
                    ...
                    $newstring = $htm =  $project; //$this->__url_get_contents($url);
                    //$substr = '<head>';
